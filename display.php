...
$renderTable = function ($basePath, $currentPage, $targetViewName = null) use ($markdownDir, $Spyc, $Parsedown) {
    ...
    if (isset($baseData['views'])) {
        foreach ($baseData['views'] as $idx => $view) {
            if ($targetViewName && strcasecmp($view['name'] ?? '', $targetViewName) === 0) {
                $viewIndex = $idx; break;
            }
        }
    }
    ...
    if (is_dir($scanDir)) {
        ...
        foreach ($it as $file) {
            if ($file->isFile() && strtolower($file->getExtension()) === 'md') {
                if (realpath($file->getPathname()) !== realpath($currentPage) && $file->getFilename() !== 'bio.md') {
    ...
    // SMART PROPERTY FINDER: Strips prefixes like 'formula.' or 'file.'
    $findProp = function ($props, $id) {
        $cleanId = strtolower(str_replace(['formula.', 'file.', ' ', '_', '-'], '', $id));
    ...
    $tableHtml = "<div class='base-embed'><table class='bases-table'><thead><tr>";
    foreach ($order as $colId) {
        $colName = str_replace(['formula.', 'file.', '.', '_'], ['', '', ' ', ' '], $colId);
    ...
    foreach ($mdFiles as $mdFile) {
        ...
        $relPath = ltrim(str_replace([realpath($markdownDir), '.md', '\\'], ['', '', '/'], realpath($mdFile)), '/');
        $finalUrl = create_wiki_url($relPath);

        $raw = file_get_contents($mdFile);
        $props = preg_match('/^---\s*([\s\S]*?)\s---/u', $raw, $m) ? Spyc::YAMLLoad($m[1]) : [];

        $tableHtml .= "<tr onclick=\"if(!event.target.closest('a')){window.location='$finalUrl';}\" style='cursor:pointer;'>";
        foreach ($order as $propId) {
            ...
            if (is_array($val)) {
                $pills = array_map(function ($i) use ($markdownDir, $Parsedown) {
                    $txt = (string)(is_array($i) ? implode(', ', $i) : $i);
                    return "<span class='prop-pill'>" . render_wiki_markup_html($Parsedown->line($txt), $markdownDir, $Parsedown, true) . "</span>";
                }, $val);
